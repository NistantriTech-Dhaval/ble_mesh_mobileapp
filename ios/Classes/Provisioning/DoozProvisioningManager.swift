...
class DoozProvisioningManager: NSObject {
    ...
    func didDeliverData(_ data: Data, ofType type: PduType){
        guard let _provisioningManager = self.provisioningManager,
              let _provisioningBearer = self.provisioningBearer else {
            print("Provisioning manager or bearer not available")
            return
        }
