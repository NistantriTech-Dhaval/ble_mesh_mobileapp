...
class DoozMeshManagerApi(context: Context, binaryMessenger: BinaryMessenger) : StreamHandler, MethodChannel.MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "loadMeshNetwork" -> {
                println("loadMeshNetwork method called")
                loadMeshNetwork()
                result.success(null)
            }
            ...
            "sendValueVendorModel" -> {
                val modelName = call.argument<String>("modelName")!! // optional, for logging
                val modelId = call.argument<Int>("modelId")!!
                val companyId = call.argument<Int>("companyId")!!
                val opCode = call.argument<Int>("opCode")!!
                val keyIndex = call.argument<Int>("keyIndex")!!
                val parametersList = call.argument<List<Int>>("parameters")!!
                val parameters = parametersList.map { it.toByte() }.toByteArray()
                val address = call.argument<Int>("address")!!
                val currentMeshNetwork = mMeshManagerApi.meshNetwork!!
